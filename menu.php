...
<body>
		...
		<?php
        	...
        	while($row = mysqli_fetch_array($run_data))
        	{
				...
				<td class='text-center'>
					<a href='menueditform.php?id=$id'><i class='halflings-icon edit'></i><span class='hidden-tablet'></span></a>
					
				</td>

				<td class='text-center'>
					<a href='menudelete.php?id=$id'><i class='halflings-icon trash'></i><span class='hidden-tablet'></span></a>
 ...
 $id = 1;
$get_data = "SELECT * FROM desmenu WHERE id = $id";
$run_data = mysqli_query($con,$get_data);
...
while($row = mysqli_fetch_array($run_data))
{
	...
	echo "
      ...
      <div class='modal-body'>
        <form action='desmenuedit.php?id=$id' method='post'>
        	
        	<div class='form-group'>
        		<label>Description</label>
        		<textarea id='w3review' name='description' rows='4' cols='50'>$description</textarea>
        	</div>
